...
public class CrateChallenge : MonoBehaviour
{
    ...
    private IEnumerator Desintegrar(Caja caja)
    {
        ...
        {
            caja.efecto.Preparar(shaderDesintegracion, colorBorde, 0f);
            yield return caja.efecto.Animar(0f, 1f, duracionDesintegracion);
        }
        caja.objeto.SetActive(false);
        caja.efecto.Restaurar();
        desaparecidas++;
    ...
    private IEnumerator AparecerTodas()
    {
        ...
        foreach (Caja caja in cajas.Values)
        {
            ...
            if (shaderAparicion != null)
                caja.efecto.Preparar(shaderAparicion, new Color(2f, 0.4f, 4f, 1f), 1f);
        }
        if (shaderAparicion != null)
        {
            float tiempo = 0f;
