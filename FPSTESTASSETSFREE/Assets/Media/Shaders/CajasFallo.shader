    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                float pattern = saturate(frac(input.positionOS.y * 1.5) * 0.85 + noise * 0.15);
                float remaining = pattern - lerp(-0.12, 1.12, _Progress);
                clip(remaining);
                ...
                half3 energy = _EdgeColor.rgb;
                return half4(baseColor * lighting * (1.0 - _Progress) + energy * edge * flicker, 1);
            }
            ENDHLSL
        }
    }
